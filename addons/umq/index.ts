...
import type { TSchema } from "../../core/src";
import type { AzureServiceBusConfig } from "./azure.js";
import type { RabbitMQConfig } from "./rabbitmq";

export interface UmqProvider {
  publish(event: string, data: { event: string; payload: unknown }): Promise<void>;
  subscribe(event: string | string[], handler: (data: { event: string; payload: unknown }) => void): Promise<void>;
  close(): Promise<void>;
