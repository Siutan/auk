...
See `dlq-example.ts` for a complete working example that demonstrates:

- Setting up DLQ configuration
- Subscribing to events with automatic DLQ handling
- Subscribing to DLQ messages
- Publishing test messages
- Retrieving DLQ messages

### 🎯 Automatic DLQ Example

See `automatic-dlq-example.ts` for the enhanced developer experience:

- **Zero-config DLQ**: DLQ is automatically handled when you subscribe to events
- **Auk Framework Integration**: Shows how to use DLQ with the Auk framework
- **Custom DLQ Handlers**: Demonstrates optional custom DLQ handlers
- **Type Safety**: Shows how to use DLQ with typed events
- **Direct Usage**: Also shows direct AukBus usage without the framework

## Error Handling

- **Connection Errors**: All connection and JetStream errors are logged to console
- **DLQ Errors**: Errors in DLQ processing are logged but don't affect the main message flow
- **Configuration Errors**: Invalid DLQ configuration will throw descriptive errors

## Best Practices

1. **🎯 Use Automatic DLQ**: Leverage the automatic DLQ handling for the best developer experience
2. **Monitor DLQ**: Regularly check DLQ messages to identify and fix processing issues
3. **Set Appropriate Limits**: Configure `maxDeliver` based on your retry requirements
4. **Custom DLQ Handlers**: Implement custom DLQ handlers for business-specific error handling
5. **Stream Management**: Consider stream retention policies for DLQ streams
6. **Error Logging**: Use the error information in DLQ messages for debugging
7. **Type Safety**: Use typed events with the Auk framework for better error handling
